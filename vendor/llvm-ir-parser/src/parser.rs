...
struct Parser<'src> {
    ...
    /// values before the first block, i.e. the argument count). Incoming references
    /// to this name must be redirected to `entry_block_name`.
    entry_block_llvm_name: Option<String>,
}
...
impl<'src> Parser<'src> {
    ...
    fn parse_module(&mut self) -> Result<(), ParseError> {
        loop {
            match self.lex.peek()? {
                ...
                Token::Kw(Keyword::Attributes) => {
                    self.skip_attributes_def()?;
                }
                Token::Bang => {
    ...
    fn parse_global_or_function(&mut self) -> Result<(), ParseError> {
        // @name = [linkage] [unnamed_addr] [addrspace(N)] (global|constant) type [initializer]
        ...
        match self.lex.peek()? {
            Token::Kw(Keyword::Global) | Token::Kw(Keyword::Constant) => {
                ...
                // Skip trailing attributes like `, align N`, `, section "..."`
    ...
    fn parse_function_type(&mut self, ret: TypeId) -> Result<TypeId, ParseError> {
        let (params, variadic) = self.parse_function_type_signature()?;
        Ok(self.ctx.mk_fn_type(ret, params, variadic))
    }

    /// Parse `(type, type, ...)` and return the parameter types and variadic flag.
    fn parse_function_type_signature(&mut self) -> Result<(Vec<TypeId>, bool), ParseError> {
    ...
    fn parse_function(&mut self, is_declaration: bool) -> Result<(), ParseError> {
        ...
        // References to this numeric name must be redirected to the block's actual
        // label (which is the numeric name for a bare entry block, or an explicit
        // label otherwise).
        let entry_llvm_name = args.len().to_string();
        self.entry_block_name = Some(entry_llvm_name.clone());
        self.entry_block_llvm_name = Some(entry_llvm_name);
    ...
    fn parse_block(&mut self) -> Result<(), ParseError> {
        ...
        // A bare entry block uses the LLVM numeric name (argument count).
        let default_entry_name = self.entry_block_llvm_name.clone().unwrap_or_else(|| "0".to_string());
        let bb_name = match self.lex.peek()? {
            Token::LocalIdent(name) => {
                // Check if the local ident is followed by ':' (block label)
                // or '=' (instruction result) to avoid consuming instruction
                // result names as block labels.
                let name_clone = name.clone();
                let n = self.lex.expect_local_ident()?;
                if self.lex.eat(&Token::Colon) {
                    // It's a block label.
                    n
                } else if self.lex.eat(&Token::Equal) {
                    // It's an instruction result, not a block label.
                    // Put back '=' then the ident so parse_instruction
                    // sees them in order: LocalIdent, then =.
                    self.lex.put_back(Ok(Token::Equal));
                    self.lex.put_back(Ok(Token::LocalIdent(name_clone)));
                    default_entry_name
                } else {
                    return Err(self.err(format!(
                        "expected ':' or '=' after local ident '{}'", n
                    )));
                }
            }
            Token::IntLit(n) => {
        ...
        // The first parsed block is the entry block. If it has an explicit label,
        // update entry_block_name so that later references to the numeric LLVM
        // name (equal to the argument count) are redirected here.
        if func.blocks.is_empty() {
            self.entry_block_name = Some(bb_name.clone());
        }
    ...
    fn parse_instr_kind(&mut self) -> Result<(InstrKind, TypeId), ParseError> {
        match self.lex.peek()? {
            ...
            | Token::Kw(Keyword::Notail) => {
                ...
                let explicit_fn_ty = if matches!(self.lex.peek()?, Token::LParen) {
                    Some(self.parse_function_type_signature()?)
                } else {
                    None
                };
                // Callee.
                ...
                self.lex.expect(&Token::RParen)?;
                // Build a function type from what we know, preferring the explicit signature.
                let callee_ty = if let Some((params, var)) = explicit_fn_ty {
                    self.ctx.mk_fn_type(ret_ty, params, var)
                } else {
                    let param_tys: Vec<TypeId> = args.iter().map(|a| self.type_of_vref(*a)).collect();
                    self.ctx.mk_fn_type(ret_ty, param_tys, is_variadic)
                };
                Ok((
                    InstrKind::Call {
                        tail,
    ...
    /// Like `parse_typed_value`, but skips parameter attribute keywords/words
    /// between the type and the value. Used for call/invoke arguments.
    fn parse_call_arg(&mut self) -> Result<(ValueRef, TypeId), ParseError> {
        let ty = self.parse_type()?;
        // Skip parameter attrs: `noundef`, `nonnull`, `signext`, `%N` alignment, etc.
        loop {
            match self.lex.peek()? {
                Token::Comma
                | Token::RParen
                ...
                // Known parameter attribute words (bare identifiers).
                Token::LocalIdent(name) => {
                    let name = name.clone();
    ...
    /// Truncate a signed integer to the bit width of `ty` and return its
    /// two's-complement representation as an unsigned value.
    fn truncate_int_to_type(&self, n: i64, ty: TypeId) -> Result<u64, ParseError> {
        match self.ctx.get_type(ty) {
            ...
            _ => Err(self.err(format!("expected integer type, got {:?}", ty))),
        }
    }
    ...
    fn parse_value(&mut self, ty: TypeId) -> Result<ValueRef, ParseError> {
        match self.lex.peek()? {
            ...
            Token::IntLit(_) | Token::UIntLit(_) => {
                // Preserve the signed value so that negative constants like -8 are
                // stored with the correct two's-complement bit pattern for `ty`.
                let n = self.lex.expect_int_lit()?;
                let val = self.truncate_int_to_type(n, ty)?;
                let c = self.ctx.const_int(ty, val);
                Ok(ValueRef::Constant(c))
            }
            Token::BigIntLit(_) => {
                let s = match self.lex.next()? {
                    Token::BigIntLit(s) => s,
                    _ => unreachable!(),
                };
                ...
                if is_negative {
                    ...
                    let mut carry = 1u64;
                    for w in &mut words {
                        let (neg, c) = (!*w).overflowing_add(carry);
                        *w = neg;
                        carry = c as u64;
                    }
                    if carry != 0 {
        ...
        Token::StringLit(_) => {
            let s = self.lex.expect_string_lit()?;
            // String literals like c"hello\00" represent arrays of i8.
            // Decode escape sequences and create a constant array.
            let bytes = decode_string_lit(&s);
            let elt_ty = self.ctx.i8_ty;
            let elems: Vec<ConstId> = bytes
                .iter()
                .map(|&b| self.ctx.const_int(elt_ty, b as u64))
                .collect();
        ...
        Token::Kw(Keyword::Getelementptr) => {
            ...
            let (base_ty, ptr, indices) = if self.lex.eat(&Token::LParen) {
                let base_ty = self.parse_type()?;
                self.lex.expect(&Token::Comma)?;
                let ptr_ty = self.parse_type()?; // pointer type
                let ptr = self.parse_value(ptr_ty)?;
                let mut indices = Vec::new();
                while self.lex.eat(&Token::Comma) {
                    let (idx, _) = self.parse_typed_value()?;
                    indices.push(idx);
            ...
            let ptr_ty = self.ctx.ptr_ty;
            let c = self.ctx.push_const(ConstantData::GetElementPtr {
                ty: ptr_ty,
                inbounds,
                base_ty,
                ptr,
    ...
    fn resolve_local(&mut self, name: &str) -> Result<ValueRef, ParseError> {
    ...
    fn get_or_create_block(&mut self, name: &str) -> Result<BlockId, ParseError> {
        ...
        // Redirect references to the entry block's LLVM numeric name to its
        // actual label (e.g. a bare entry block uses the argument count, but an
        // explicit label like "entry:" overrides it).
        let name = if Some(name.to_string()) == self.entry_block_llvm_name {
            self.entry_block_name.as_deref().unwrap_or(name)
        } else {
            name
        };
    ...
    fn skip_trailing_fn_attrs(&mut self) -> Result<(), ParseError> {
        ...
        loop {
            match self.lex.peek()? {
                ...
                | Token::Kw(Keyword::Target)
                | Token::Kw(Keyword::Source)
                | Token::Kw(Keyword::Attributes) => break,
                Token::Hash => {
                    self.lex.next()?;
                    self.lex.next()?;
                }
    ...
    fn skip_global_trailing_attrs(&mut self) -> Result<(), ParseError> {
        // Skip `, align N`, `, section "..."`, etc. after a global definition.
        loop {
            if !self.lex.eat(&Token::Comma) {
                break;
            }
            // Consume the attribute keyword and any optional value.
            match self.lex.peek()? {
                Token::Kw(Keyword::Align) => {
                    self.lex.next()?;
                ...
                Token::LocalIdent(_) => {
                    self.lex.next()?;
                    // skip optional string value
                    if matches!(self.lex.peek()?, Token::StringLit(_)) {
                        self.lex.next()?;
                    }
                }
                Token::Kw(_) => {
                ...
                _ => {
                    let t = self.lex.next()?;
                    return Err(self.err(format!("unexpected token in global trailing attrs: {:?}", t)));
                }
            }
        }
        Ok(())
    ...
    fn skip_attributes_def(&mut self) -> Result<(), ParseError> {
        // Skip `attributes #N = { ... }` definitions.
        self.lex.expect_kw(&Keyword::Attributes)?;
            ...
            match self.lex.peek()? {
                ...
                Token::LocalIdent(name) => {
                    ...
                    if KNOWN_ATTRS.contains(&name.as_str()) {
                        ...
                        //   byval(T), byref(T), preallocated(T), etc.
                        ...
                        // A local identifier that is not a known attribute is the
                        // parameter name (e.g. `%x` or `%0`).
                        break;
                    }
                }
                _ => {
    ...
    fn parse_metadata_value_text(&mut self) -> Result<String, ParseError> {
        ...
        loop {
            ...
            if depth == 0 {
                ...
                // A keyword that cannot start a metadata value signals the end of
                // the attachment (e.g. the next instruction opcode like `switch`).
                if matches!(peek, Token::Kw(kw) if !matches!(kw, Keyword::Null | Keyword::False | Keyword::True)) {
                    break;
                }
                if matches!(peek, Token::Bang) && !matches!(prev.as_ref(), Some(Token::Bang)) {
                    break;
                }
                if matches!(peek, Token::IntLit(_) | Token::UIntLit(_))
                    && !matches!(prev.as_ref(), Some(Token::Bang))
                {
                    break;
                }
...
/// Decode an LLVM IR string literal into raw bytes.
///
/// The string returned by the lexer's `read_string_literal` already has hex
/// escapes (`\NN`) decoded to their byte values as Rust `char` code points.
/// All such code points are in 0..=255, so `c as u8` recovers the original byte.
fn decode_string_lit(s: &str) -> Vec<u8> {
