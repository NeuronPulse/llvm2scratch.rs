...
impl<'a> Printer<'a> {
    ...
    fn write_const_value(&self, out: &mut String, id: ConstId) {
        match self.ctx.get_const(id) {
            ...
            ConstantData::GetElementPtr { inbounds, base_ty, ptr, indices, .. } => {
                if *inbounds {
                    out.push_str("getelementptr inbounds (");
                } else {
                    out.push_str("getelementptr (");
                }
                self.write_type(out, *base_ty);
                out.push_str(", ");
                self.write_const_gep_value(out, *ptr);
                for idx in indices {
                    out.push_str(", ");
                    self.write_const_gep_value(out, *idx);
                }
                out.push(')');
            }
        }
    ...
    /// Print a value that appears inside a constant GEP expression. These are
    /// always constants or global references, so no function context is needed.
    fn write_const_gep_value(&self, out: &mut String, vref: ValueRef) {
        match vref {
            ...
            ValueRef::Global(id) => {
                write!(out, "ptr @g{}", id.0).unwrap();
            }
            _ => {
                write!(out, "ptr %v{}?", 0).unwrap();
            }
        }
    }
